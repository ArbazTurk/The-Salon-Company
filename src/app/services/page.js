...
export default function ServicesPage() {
  ...
  // Correction is in this useEffect block
  useEffect(() => {
    // We re-initialize WOW.js every time the activeCategory changes.
    // This makes sure that the animation library scans the newly added
    // service cards and applies the animation to them.
    import('wowjs').then(WOW => {
      new WOW.default.WOW({
        live: false,
      }).init();
    });
  }, [activeCategory]); // <-- Added [activeCategory] as a dependency
